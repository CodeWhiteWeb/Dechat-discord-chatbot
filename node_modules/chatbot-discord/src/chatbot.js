...
class Chatbot {
  constructor(ops = defaultOptions) {
    this.ops = ops;
  }
  ...
  async chat(message) {
    if (!message) {
      throw new Error(
        "[Chatbot DIscord API] You need to provide a message to reply to!"
      );
    }
    ...
    let res = await fetch(
      `https://chatbot-api.vercel.app//?message=${encodeURIComponent(
        message
      )}&${url}`
    ).catch((e) => {
    ...
    const translation = await translate(response.message, {
      to: ops.language,
    }).catch((e) => {
      throw new Error(`Ran into an Error. ${e}`);
    });
    ...
    if (!response) {
      throw new Error(
        "[Chatbot API] API is unavailable. Please try again later. Or contact the developer from contact form at https://codewhiteweb.cf/#contact"
      );
    }
